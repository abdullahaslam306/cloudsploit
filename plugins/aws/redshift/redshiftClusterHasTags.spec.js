...
const redshiftClusterHasTags = require('./redshiftClusterHasTags');
...
describe('redshiftClusterHasTags', function () {
    describe('run', function () {
        it('should PASS if Redshift cluster has tags', function (done) {
            const cache = createCache([describeClusters[0]]);
            redshiftClusterHasTags.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].region).to.equal('us-east-1');
                expect(results[0].message).to.include('Redshift cluster has tags');
                done();
            });
        });

        it('should FAIL if Redshift cluster has no tags', function (done) {
            const cache = createCache([describeClusters[1]]);
            redshiftClusterHasTags.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].region).to.equal('us-east-1');
                expect(results[0].message).to.include('Redshift cluster has no tags')
                done();
            });
        });
